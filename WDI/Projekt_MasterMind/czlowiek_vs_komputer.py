...
class Kod(object):
    ...
    def kod_gry(self, aktualny_kolor):
        ...
        #self.kod_los = [1, 2, 2, 4]

        # losowanie kodu
        if self.kod_los == [] or self.kod_los == [0, 0, 0, 0]:
            for kos_i in range(4):
                random.seed()
                if aktualny_kolor == 6:
                    losek = random.randint(1, 6)
                if aktualny_kolor == 7:
                    losek = random.randint(1, 7)
                if aktualny_kolor == 8:
                    losek = random.randint(1, 8)
                self.kod_los[kos_i] = losek


        # wypełnainie tablicy aktualna_podpowiedz
